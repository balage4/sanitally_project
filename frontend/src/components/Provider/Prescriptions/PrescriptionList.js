...
export default function PrescriptionList({ user }) {
  ...
  useEffect(async () => {
    try {
      ...
      if (user.role === 'admin') {
        prescriptionResponse = await fetchWithAuth(`${backend.endpoint}/prescriptions`, user.token);
      } else {
        prescriptionResponse = await fetchWithAuth(`${backend.endpoint}/prescriptions/${user.email}`, user.token);
